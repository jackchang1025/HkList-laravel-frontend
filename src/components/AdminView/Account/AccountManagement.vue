<template>
    ...
    <el-form :inline="true" :model="searchForm">
      <el-form-item label="关键词">
        <el-input
          ...
          placeholder="搜索用户名/CID"
          clearable
          @keyup.enter="handleSearch"
        />
      </el-form-item>
      <el-form-item label="关联企业账号">
        <el-select 
          v-model="searchForm.enterprise_account_id"
          clearable 
          placeholder="选择企业账号"
          style="width: 220px"
        >
  ...
  >
    ...
    <el-table-column prop="enterprise_account" label="关联企业账号" width="200px">
      <template #default="{ row }">
        <span v-if="!row.edit">
          {{ row.enterprise_account ? `${row.enterprise_account.name}` : '未关联' }}
        </span>
        <el-select 
          v-if="row.edit" 
          v-model="row.enterprise_account_id"
          clearable
          placeholder="选择企业账号"
        >
          <el-option 
...
const searchForm = ref({
  keyword: '',
  enterprise_account_id: undefined as number | undefined
})

// 重置搜索
const resetSearch = () => {
  searchForm.value.keyword = ''
  searchForm.value.enterprise_account_id = undefined
  handleSearch()
}
...
const getAccounts = async () => {
  try {
    ...
    const res = await AccountApi.getAccount({ 
      ...
      enterprise_account_id: searchForm.value.enterprise_account_id
    })
    res.data.data = res.data.list.map((v) => {
